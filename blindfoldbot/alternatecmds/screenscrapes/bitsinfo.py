...
async def get_bitleader_info_from_twitch_page_async(channel_to_check):
    twbrowser = await pyppeteer.launcher.launch()
    twpages = await twbrowser.pages()
    twpage = twpages[0]
    await twpage.goto('https://www.twitch.tv/'+ channel_to_check)
    ...
    return alternatecmds.screenscrapes.html_bitleader.perform_parse(channel_content)

        
def get_bitleader_info_from_twitch_page(channel_to_check):
    return asyncio.get_event_loop().run_until_complete(get_bitleader_info_from_twitch_page_async(channel_to_check))
